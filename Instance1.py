...
for _ in tqdm(range(runs)):
    ...
    for c in range(total_cases):
        if cases[c] == 'UCB1':
            iter_regret = ucb1(arms_reward_mean, arms_reward_var, samples)

        elif cases[c] == 'UCB-CV':
            iter_regret = ucb_cv(arms_mean, cvs_mean, arms_sd, cvs_sd, samples)

        elif cases[c] == 'UCB-LZVCV':
            iter_regret = ucb_lzvcv(arms_reward_mean, arms_reward_var, samples)

        elif cases[c] == 'UCB-QZVCV':
            iter_regret = ucb_qzvcv(arms_reward_mean, arms_reward_var, samples)
...
xlabel = "Rounds"
ylabel = "Regret"
file_to_save = "Instance1.png"
title = "Gaussian Distribution - Average Regret After 100 Experiments"
