...
def ucb1(arms_reward_mean, arms_reward_scale, T):
    ...
    for k in range(K):
        arm_rewards[k] += np.random.logistic(arms_reward_mean[k],arms_reward_scale[k],1)[0]
    ...
    for t in range(K, T):
        ...
        arm_rewards[I_t] += np.random.logistic(arms_reward_mean[I_t],arms_reward_scale[I_t],1)[0]
...
def ucb_cv(mu, omega, arms_scale, cvs_scale, T):
    ...
    mu: mean for V
    omega: mean for W
    arms_scale: scale value for V
    cvs_scale: scale value for W
    T: number of rounds (int)
    '''
    ...
    for k in range(3 * K):
        # Samples
        k = k % K
        arm_cv_value = np.random.logistic(omega[k], cvs_scale[k], 1)[0]
    ...
    for t in range(3 * K, T):
        ...
        arm_cv_value = np.random.logistic(omega[I_t], cvs_scale[I_t], 1)[0]
...
def ucb_lzvcv(arms_reward_mean, arms_reward_scale, T):
    ...
    ============================================
    arms_reward_mean: mean reward for each arm
    arms_reward_scale: scale value of the reward for each arm
    T: number of rounds (int)
    '''
    ...
    for k in range(3*K):
        ...
        arm_reward_scale = arms_reward_scale[k]
        arm_reward = np.random.logistic(arm_reward_mean,arm_reward_scale,1)[0]
        # Construct the LZVCV from evaluations
        arm_cv_value = -1/2 * ((np.exp(arm_reward_mean/arm_reward_scale)-np.exp(arm_reward/arm_reward_scale))/(arm_reward_scale*(np.exp(arm_reward_mean/arm_reward_scale)+np.exp(arm_reward/arm_reward_scale))))
    ...
    for t in range(3*K, T):
        ...
        arm_reward_scale = arms_reward_scale[I_t]
        arm_reward = np.random.logistic(arm_reward_mean,arm_reward_scale,1)[0]
        # Construct the LZVCV from evaluations
        arm_cv_value = -1/2 * ((np.exp(arm_reward_mean/arm_reward_scale)-np.exp(arm_reward/arm_reward_scale))/(arm_reward_scale*(np.exp(arm_reward_mean/arm_reward_scale)+np.exp(arm_reward/arm_reward_scale))))
...
def ucb_qzvcv(arms_reward_mean, arms_reward_scale, T):
    ...
    for k in range(4 * K):
        ...
        arm_cv1_value = -1/2 * ((np.exp(arm_reward_mean/arm_reward_scale)-np.exp(arm_reward/arm_reward_scale))/(arm_reward_scale*(np.exp(arm_reward_mean/arm_reward_scale)+np.exp(arm_reward/arm_reward_scale))))
        arm_cv2_value = (-1/2 * ((np.exp(arm_reward_mean/arm_reward_scale)-np.exp(arm_reward/arm_reward_scale))/(arm_reward_scale*(np.exp(arm_reward_mean/arm_reward_scale)+np.exp(arm_reward/arm_reward_scale))))) * arm_reward - 1/2
...
cvs_scale = np.zeros(arms)
arms_rewards_scale = np.zeros(arms)
for k in range(arms):
    arms_mean[k] = max_arm_mean - (k*arm_gap)
    arms_scale[k] = scale
    cvs_mean[k] = max_cv_mean - (k*cv_gap)
    cvs_scale[k] = scale
    arms_rewards_scale[k] = scale
...
for _ in tqdm(range(runs)):
    ...
    for c in range(total_cases):
        if cases[c] == 'UCB1':
            iter_regret = ucb1(arms_reward_mean, arms_rewards_scale, samples)

        elif cases[c] == 'UCB-CV':
            iter_regret = ucb_cv(arms_mean, cvs_mean, arms_scale, cvs_scale, samples)

        elif cases[c] == 'UCB-LZVCV':
            iter_regret = ucb_lzvcv(arms_reward_mean, arms_rewards_scale, samples)

        elif cases[c] == 'UCB-QZVCV':
            iter_regret = ucb_qzvcv(arms_reward_mean, arms_rewards_scale, samples)
...
xlabel = "Rounds"
ylabel = "Regret"
file_to_save = "Instance3.png"
title = "Logistic Distribution - Average Regret After 100 Experiments"
